...
| `optimize.py` | BFGS on the predicted property in latent space from one graph's encoding, under an optional prior penalty `(λ/2)·‖x‖²`, in float64 on the cpu; the path is every accepted iterate. Its CLI prints each path, for reading how the optimizer behaves |
