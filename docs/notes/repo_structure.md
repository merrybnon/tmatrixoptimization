...
| `visualize.py` | diagnostic figures into the run's `figures/`, and decoded latent traversals (global dim, sorted-160 PC1 and PC2) into `figures/interpolation_and_traversals/`, each stamped with a provenance footer — checkpoint epoch and score, drop, split seed, git hash, timestamp — so a figure is self-describing wherever it ends up |
