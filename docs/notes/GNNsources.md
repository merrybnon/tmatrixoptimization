...
**Wagstaff et al., *On the Limitations of Representing Functions on Sets*, ICML 2019.** arXiv 1901.09006. The sharp edge on Deep Sets: a sum- or mean-decomposable function needs the per-element encoding φ to be at least as wide as the set size to represent every continuous invariant function on sets that big. This is what sizes the pooling in `models.py`. The original `z.mean(1)` over d_latent = 8 with n = 20 was below the bound before its missing nonlinearity was even counted; φ at predictor_hidden = 64 clears it, so the `deepsets` mode is not capacity-limited and `attention` is a claim about inductive bias rather than about expressiveness. Follow-up work relaxes this for particular function families, but the bound is the right default assumption.

**Lee et al., *Set Transformer*, ICML 2019.** Attention-based pooling (PMA), a learned alternative to mean or sum pooling. One seed query attending over the elements gives a convex combination of them, so it is invariant like the mean but weights elements instead of averaging them. Our `pooling: attention` mode, to be settled against `deepsets` by sweeping the two.
